...
import { deleteClassById, getAllStudents } from "@/lib/api-service";
...
export const ClassesTable = ({
	...
	const getStudentNames = (studentIds: string[]) => {
		return studentIds
			.map((id) => students.find((s) => s._id === id)?.name || "Unknown")
			.join(", ");
	};
			...
			<Card>
				...
				<CardContent>
					<div className="h-32 flex items-center justify-center">
						<div className="animate-pulse text-education-primary">
							Loading...
						</div>
					</div>
				</CardContent>
		...
		<Card>
			<CardHeader>
				...
				<CardDescription>
					...
					{filteredClasses.length === 1 ? "class" : "classes"} scheduled
				</CardDescription>
			</CardHeader>
			<CardContent>
				{enableSearch && (
								...
								{filteredClasses.length === 0 ? (
									...
									filteredClasses.map((classItem) => (
										...
										>
											...
											<TableCell>{classItem.room || "TBD"}</TableCell>
											{showDetails && (
												<TableCell>
													{classItem.category && (
